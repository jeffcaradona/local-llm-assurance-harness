...
One configured model makes sequential calls. Defaults are 8 model calls, 6 model-requested tools, and an overall 600,000 ms deadline after admission, covering root validation through artifact persistence. Configured integer ranges are respectively `1..100`, `0..100`, and `1..2147483647`; per-call model and tool timeouts still apply. There is no delegation, shell, mutation, infrastructure adapter, or background-job surface.
...
Evidence IDs are assigned cumulatively. Identity includes capability, normalized path, line range, retained redacted content, original byte count, and truncation state. Repeated identical records reuse IDs; changed records receive new IDs. Different capabilities can produce distinct records for the same source. This is not an atomic snapshot, and changes outside retained excerpts need not be detectable.

The selected root may itself be a symlink and is resolved to its canonical directory. Symlink components beneath the root remain blocked; canonical containment and file identity checks do not provide an OS sandbox against concurrent mutation.
...
Sanitized metadata-only manifests are published as success markers outside the reviewed root. Investigation metadata includes counts, timing, safe turn/status records, stop reasons, and evidence IDs, not raw actions or source content. Failures emit stable errors and safe stage/count metadata when available on stderr, not a successful report.

Persistence receives the active cancellation signal and stages files in a unique directory inside the canonical artifact output directory, outside the reviewed repository. It publishes the optional replay first and manifest last using exclusive hard links, preserving pre-existing files. Abort or failure triggers best-effort removal of this run's staging and newly published artifacts. The orchestrator can reject promptly and release admission while non-cancellable filesystem IO settles; its continuation then performs cleanup. This is not a crash-atomic multi-file transaction: cleanup failures or process termination can leave residual files.
...
The investigation bundle uses `format: "investigation"` and recording version `1.1.0`; the model action protocol remains `1.0.0`. Replay still accepts strict investigation `1.0.0` recordings and unchanged legacy single-pass bundles. Replay validates structure and digest, then feeds recorded exchanges through the shared execution core and compares contexts, invocation order, outcomes, evidence, counters, and final review. Recorded elapsed times are checked for consistency with the budget, not reproduced by sleeping or replaying real-time deadlines. Invalid records fail with `E_REPLAY_INVALID`; execution mismatches fail with `E_REPLAY_MISMATCH`.

Live model actions are strictly validated before dispatch. Original arguments, including operator seed paths and searches, reach the capability policy unchanged through a separate ephemeral exchange parameter. Only sanitized copies enter recorded actions, outcomes, evidence, subsequent contexts, or artifacts; protocol discriminators and allowlisted tool names are not redacted. File seeds are sorted and deduplicated before redaction, preserving distinct original paths even when their recorded representations collide. Search seeds retain operator order and duplicates.

Version `1.1.0` requires boolean `argumentsRedacted` on model and tool events (false for final actions and argument-free discovery), plus parallel `seedArgumentsRedacted.selectedFiles` and `.searches` arrays. True means redaction changed the original arguments. Offline reconstruction preserves recorded seed order and multiplicity and never supplies recorded representations to live capabilities. Changed strings may exceed the original argument/response size limits or no longer satisfy executable path syntax; replay validates their strict action shape and types, not those original-string constraints. Unchanged arguments retain strict protocol validation. Metadata must agree across seed/action/operation records. Replay cannot recover or revalidate redacted originals, prove the metadata truthful, or establish that a recorded representation was an executable command.

Model events also record the original serialized action's `responseBytes` and a boolean `responseRedacted` (covering final review redaction as well as tool arguments). Replay always checks the original count against the configured response budget and a structural minimum; unchanged responses must match their actual serialized byte count. Both live recording and replay bound sanitized action representations to the larger of `maxPromptChars` and 32 times `maxResponseBytes`, allowing ordinary masking expansion without accepting unlimited recorded responses.

Legacy investigation file seeds must remain sorted and unique in their stored sanitized form. For `1.1.0`, replay checks sorting and uniqueness of the unchanged-file subsequence only: original ordering across redacted paths is unrecoverable, so the recording's order is preserved rather than sorted again. Recording metadata never enters model contexts or the action protocol.
