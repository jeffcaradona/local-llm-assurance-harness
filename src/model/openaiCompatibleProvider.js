  ...
  return {
    async complete({ systemPrompt, userPrompt, responseSchema, signal }) {
      ...
      if (inputChars > config.maxPromptChars) {
        throw new HarnessError(
          'E_INPUT_BUDGET_EXCEEDED',
          'Prompt exceeded configured character budget.',
          {
            inputChars,
      ...
      const combined = AbortSignal.any(
        [timeoutController.signal, signal].filter(Boolean)
      );
      ...
      try {
        ...
        if (typeof content !== 'string') {
          throw new HarnessError(
            'E_MODEL_RESPONSE_SHAPE',
            'Model response omitted text content.'
          );
        }
        ...
        try {
          ...
          throw new HarnessError(
            'E_MODEL_OUTPUT_NOT_JSON',
            'Model output content was not JSON.'
          );
        }
      } catch (error) {
        ...
        throw new HarnessError('E_MODEL_TRANSPORT', 'Model transport failed.', {
          cause: error?.message,
        });
      } finally {
        clearTimeout(timer);
