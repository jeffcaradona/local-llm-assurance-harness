...
export async function compilePromptContext({
  ...
  evidence,
  instructionFiles,
  trustedInstructions: suppliedInstructions,
  maxChars,
  signal,
}) {
  const trustedInstructions = suppliedInstructions
    ? structuredClone(suppliedInstructions)
    : [];
  if (!suppliedInstructions) {
  ...
  const instructionsText = trustedInstructions
    .map((x) => `### ${x.filePath}\n${x.content}`)
    .join('\n\n');
  ...
  const scaffoldChars =
    systemPrompt.length + userPrefix.length + noEvidenceText.length;
  if (scaffoldChars > maxChars) {
    throw new HarnessError(
      'E_PROMPT_BUDGET_EXCEEDED',
      'Review schema, trusted instructions, and request exceed prompt budget.',
      {
        maxChars,
  ...
  for (const item of [...evidence].sort((a, b) => a.id.localeCompare(b.id))) {
    ...
    if (
      systemPrompt.length + userPrefix.length + candidateText.length >
      maxChars
    ) {
