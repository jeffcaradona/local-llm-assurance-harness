...
test('context compilation is deterministic and tracks omissions', async () => {
  ...
  const baseline = await compilePromptContext({
    request: 'r',
    evidence: [],
    maxChars: 10_000,
  });
  const maxChars =
    baseline.systemPrompt.length + baseline.userPrompt.length + 100;
  ...
  assert.ok(
    context.systemPrompt.length + context.userPrompt.length <= maxChars
  );
  assert.deepEqual(
    await compilePromptContext({
      request: 'r',
...
test('context supplies the complete validator schema even with no evidence', async () => {
  ...
  const schemaText = context.systemPrompt.split(
...
test('review validator still rejects valid JSON with the wrong review shape', () => {
  assert.throws(
    () => validateReviewPayload({ concurrency_bounds: [] }),
    (error) => {
      assert.equal(error.code, 'E_REVIEW_SCHEMA_INVALID');
      assert.ok(
        error.details.errors.some(
          (item) => item.params.missingProperty === 'schemaVersion'
        )
      );
      assert.ok(
        error.details.errors.some(
          (item) => item.params.additionalProperty === 'concurrency_bounds'
        )
      );
      return true;
    }
...
test('review validator rejects unknown evidence references', () => {
  ...
  assert.throws(() => verifyEvidenceReferences(payload, ['ev-0001']), {
    code: 'E_UNKNOWN_EVIDENCE_REFERENCE',
  });
});

test('findings cannot cite omitted evidence while limitations can', () => {
  const payload = {
    schemaVersion: REVIEW_SCHEMA_VERSION,
    summary: 'x',
    decision: 'needs_attention',
  ...
  assert.throws(
    () => verifyEvidenceReferences(payload, ['ev-0001'], ['ev-0002']),
    {
      code: 'E_UNKNOWN_EVIDENCE_REFERENCE',
    }
  );
