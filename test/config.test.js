...
test('model temperature defaults to 0.2 and accepts overrides', () => {
  for (const value of [undefined, '', '  ']) {
    assert.equal(validateRuntimeConfig(resolveRuntimeConfig({ HARNESS_MODEL_TEMPERATURE: value })).model.temperature, 0.2);
  }
  assert.equal(validateRuntimeConfig(resolveRuntimeConfig({ HARNESS_MODEL_TEMPERATURE: '0' })).model.temperature, 0);
  assert.equal(validateRuntimeConfig(resolveRuntimeConfig({ HARNESS_MODEL_TEMPERATURE: '0.7' })).model.temperature, 0.7);
});

test('rejects invalid model temperatures', () => {
  for (const value of ['-0.1', '2.1', 'abc', 'Infinity']) {
    assert.throws(
      () => validateRuntimeConfig(resolveRuntimeConfig({ HARNESS_MODEL_TEMPERATURE: value })),
      { code: 'E_CONFIG_INVALID' }
    );
  }
});
...
test('model reasoning effort is unset by default and accepts known levels', () => {
  for (const value of [undefined, '', '  ']) {
    assert.equal(validateRuntimeConfig(resolveRuntimeConfig({ HARNESS_MODEL_REASONING_EFFORT: value })).model.reasoningEffort, undefined);
  }
  for (const value of ['none', 'low', 'medium', 'high']) {
    assert.equal(validateRuntimeConfig(resolveRuntimeConfig({ HARNESS_MODEL_REASONING_EFFORT: value })).model.reasoningEffort, value);
  }
});
...
test('rejects unknown model reasoning effort', () => {
  for (const value of ['off', 'NONE', 'max']) {
    assert.throws(
      () => validateRuntimeConfig(resolveRuntimeConfig({ HARNESS_MODEL_REASONING_EFFORT: value })),
      { code: 'E_CONFIG_INVALID' }
    );
  }
});
